...
void loop() { 
  ...
  if ((distance == 0.0) || (distance > _DIST_MAX)) {
      ...
      // Map the distance to LED brightness (0 to 255)
      // 100mm to 200mm: brightness goes from 255 (off) to 0 (max brightness)
      // 200mm to 300mm: brightness goes from 0 (max brightness) to 255 (off)
      if (distance <= 200.0) {
        brightness = map(distance, _DIST_MIN, 200, 255, 0);
      } else { // distance > 200.0
        brightness = map(distance, 200, _DIST_MAX, 0, 255);
      }
